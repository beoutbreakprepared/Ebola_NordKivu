...
plot_tt <- function(infected_sf, access_raster, bg_adm0, bg_adm1, lakes){
  ...
  ttmap <-
    ...
    geom_polygon(data = as_df, aes(x=long, y = lat, group=group, colour = 'Affected health\nareas'), fill = 'gray55', alpha = 0.3)+
    geom_point(data = ariwara, aes(x = long, y = lat), color = 'gray55', fill = 'gray55', alpha = 0.3, size = 1) +
    geom_point(data = ariwara, aes(x = long, y = lat), fill = 'transparent', stroke = 1, size = 1, color = 'black', shape = 21) +
...
plot_rel_map <- function(infected_sf, access_raster, bg_adm0, lakes, co){
  ...
  rel_map <-
    ...
    geom_polygon(data = as_df, aes(x=long, y = lat, group=group, colour = 'Affected health\nareas'), fill = 'gray55', alpha = 0.3, show.legend = TRUE)+
    geom_point(data = ariwara, aes(x = long, y = lat), color = 'gray55', fill = 'gray55', alpha = 0.3, size = 1) +
    geom_point(data = ariwara, aes(x = long, y = lat), fill = 'transparent', stroke = 1, size = 1, color = 'black', shape = 21) +
